...
class Huffman
{
	...
	public function buildTree($data)
	{
		...
		while (count($roots) > 1) {
			// mencari node dengan frekuensi terkecil 
			...
			// menggabungkan 2 nodes
			$node = new Node();
			$leastZero = true;
			if ($roots[$leastOften]->height > $roots[$secondLeastOften]->height)
				$leastZero = false;
			else if (
				$roots[$leastOften]->height == $roots[$secondLeastOften]->height
